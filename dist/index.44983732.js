...
function searchForMeal() {
    ...
    // Promise chaining
    fetch(`https://www.themealdb.com/api/json/v1/1/search.php?s=${searchText}`)// convert response to json
    .then((response)=>response.json())// json structure -> {meals:[]}
    .then((data)=>{
        let list = "";
        if (data.meals) // loop over every meal and add it to the list
...
function getRecipeDetails(event) {
    if (event.target.classList.contains("recipe-button")) {
        ...
        fetch(`https://www.themealdb.com/api/json/v1/1/lookup.php?i=${mealId}`).then((response)=>response.json()).then((data)=>addRecipeDetail(data.meals[0]));
    }
}
...
function addToFavourites(event) {
    ...
    let favouriteMeals;
    if (localStorage.getItem("favourites") === null) favouriteMeals = [];
    else favouriteMeals = JSON.parse(localStorage.getItem("favourites"));
    // check if the mealId is already present
    if (favouriteMeals.indexOf(mealId) !== -1) return;
