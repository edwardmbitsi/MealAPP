...
function getFavouriteMeals() {
    ...
    if (localStorage.getItem("favourites") === null) {
        favouriteMealsId = [];
        return;
    } else favouriteMealsId = JSON.parse(localStorage.getItem("favourites"));
    let content = "";
    // loop over each meal ID and fetch for meals
    favouriteMealsId.forEach((mealId)=>{
        fetch(`https://www.themealdb.com/api/json/v1/1/lookup.php?i=${mealId}`).then((response)=>response.json()).then((data)=>addfavouriteMeals(data.meals[0], content));
    });
}
...
function removeFromFavourites(event) {
    if (!event.target.classList.contains("unfavourite-button")) return;
...
//# sourceMappingURL=favourite.ffd0fdbc.js.map
